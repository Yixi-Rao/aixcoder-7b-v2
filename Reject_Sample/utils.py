...
def deduplicate_jsonl(file_path):
    ...
    print(f"Total samples: {cnt}, Unique samples: {len(unique_samples)}")
    # 将去重后的结果写回原文件
...
def find_raw_code_line(raw_code, code):
    """
    从 raw_code 中找到对应 code 的原始行
    """
    # 按行拆分 raw_code
    raw_lines = raw_code.split("\n")
    
    # 遍历每一行，逐行比较 strip 后的内容
    for line in raw_lines:
        if line.strip() == code:
            return line + "\n"  # 返回原始行内容
    
    return None  # 如果找不到匹配的行，返回 None

import pdb
def truncate(code, span=False, need_raw_code=False, codeqwen=False):
    if codeqwen:
        if "<|file_sep|>" in code:
            code = code.split("<|file_sep|>")[0]
        if "<|fim_pad|>" in code:
            code = code.split("<|fim_pad|>")[0]
        if "<EOT>" in code:
            code = code.split("<EOT>")[0]
        code = code.strip()
        return code
    raw_code = code
    if "```json" in code: # 判断有json结构
        ...
        try: # 是否是完整的json结构
            ...
            try:
                code = code.split("\"completion\": \"")[1] # 连completion都没打完整
            except:
                pass
    if "python" in code: # 判断有python结构
    ...
    code = code.strip()
    if need_raw_code:
        return find_raw_code_line(raw_code, code)
