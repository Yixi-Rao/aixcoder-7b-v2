...
def process_sample_instruction_model(sample, language):
    ...
    repo_name_prompt = f"<|repo_name|>{project_dir}\n"

    fim_task = f"<|file_sep|>{code_file_path}"
    fim_task += f"{prefix_token}{prefix} "
    fim_task += f"{suffix_token}{suffix}{mid_token}"

    prompt = repo_name_prompt + fim_task
    ...
    ret = repo_name_prompt + comment_block + fim_task

    return ret, middle

output_dir = f"/data/codeqwen_dpo"
