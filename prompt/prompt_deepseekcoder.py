...
BASE_MODEL_EOS = '<｜end▁of▁sentence｜>'
INSTRUCT_MODEL_EOT = '<|EOT|>'
...
def process_sample_instruction_model(sample, language):
    ...
    fim_task += f"<｜fim▁begin｜>{prefix}\n"
    fim_task += "<｜fim▁hole｜>\n"
    fim_task += f"{suffix}<｜fim▁end｜>"
...
output_dir = f"/data/deepseekcoder_dpo"
        ...
        with open(output_file, 'w', encoding='utf-8') as out_f:
            for line_num, line in enumerate(tqdm(f, desc=f"Processing {file_name} samples", total=total_samples), 1):
                try:
                    ...
                    prompt, middle = process_sample_instruction_model(sample, language)
                    if prompt == -1:
                        continue
                    output = middle
                    data_entry = {
